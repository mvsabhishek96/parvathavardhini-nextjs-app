...
export default function DonationForm() {
  ...
  const pickContact = async () => {
      if ('contacts' in navigator && 'select' in navigator.contacts) {
          try {
              ...
              alert('Error accessing contacts. Please enter manually.');
          }
      } else { 
          alert('Contact Picker is not supported on your browser. Please enter manually.'); 
      }
  };
  ...
  return (
    <div className="w-full max-w-md mx-auto bg-white/90 backdrop-blur-sm border border-maroon-300 p-6 rounded-2xl shadow-lg">
      ...
      <form onSubmit={handleSubmit} className="space-y-5">
        <div>
          <label className="block text-sm font-medium text-maroon-800 mb-2">Donation Type</label>
        ...
        <div>
          <label htmlFor="name" className="block text-sm font-medium text-maroon-800 mb-2">Devotee&apos;s Name</label>
          <div className="relative">
            ...
            <input 
              ...
              className="w-full p-3 border border-maroon-300 rounded-lg focus:ring-2 focus:ring-gold-500 focus:border-transparent pl-10" 
        ...
        <div>
          <label htmlFor="city" className="block text-sm font-medium text-maroon-800 mb-2">City</label>
          <div className="relative">
        ...
        <div>
          <label htmlFor="gothra" className="block text-sm font-medium text-maroon-800 mb-2">Gothra</label>
          <div className="relative">
        ...
        {donationType === 'cash' ? (
          <div>
            <label htmlFor="amount" className="block text-sm font-medium text-maroon-800 mb-2">Donation Amount (₹)</label>
            <div className="relative">
          ...
          <div>
            <label htmlFor="inKindDescription" className="block text-sm font-medium text-maroon-800 mb-2">Item Description</label>
            <div className="relative">
              ...
              <input 
                ...
                className="w-full p-3 border border-maroon-300 rounded-lg focus:ring-2 focus:ring-gold-500 focus:border-transparent pl-10" 
                placeholder="Describe donated items" 
              />
            </div>
          </div>
        )}
        ...
        <div>
          <label htmlFor="phoneNumber" className="block text-sm font-medium text-maroon-800 mb-2">Phone Number</label>
          <div className="relative">
            ...
            <input 
              ...
              className="w-full p-3 border border-maroon-300 rounded-lg focus:ring-2 focus:ring-gold-500 focus:border-transparent pl-10 pr-10" 
              placeholder="10-digit mobile number" 
            />
            <i 
              onClick={pickContact} 
              className="fas fa-address-book absolute right-3 top-1/2 transform -translate-y-1/2 cursor-pointer text-maroon-600 hover:text-gold-600 transition-colors"
            ></i>
          </div>
        </div>

        {message && (
          <div className={`p-3 rounded-lg text-center ${message.startsWith('Divine blessings') ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'}`}>
        ...
        <button 
          ...
          className="w-full bg-gradient-to-r from-saffron-500 to-saffron-600 text-white font-bold text-lg py-3 rounded-lg hover:opacity-90 transition-transform duration-200 hover:scale-105 disabled:opacity-50 disabled:cursor-not-allowed"
        >
          {isSubmitting ? (
            <>
