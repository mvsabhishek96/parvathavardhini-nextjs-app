  ...
  return (
    <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50" onClick={onClose}>
        <div className="bg-white p-6 rounded-lg shadow-xl w-full max-w-lg mx-4" onClick={e => e.stopPropagation()}>
            <h3 className="text-2xl font-bold font-display text-maroon-800 mb-4">Donation Details</h3>
            ...
            <button onClick={onClose} className="mt-6 w-full bg-gray-500 text-white py-2 rounded-lg hover:bg-gray-600 transition-colors">Close</button>
        </div>
    </div>
  );
}